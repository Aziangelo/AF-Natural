...
void main() {
  vec4 diffuse;
  vec4 glowTexture = texture2DLod(s_MatTexture, v_texcoord0, 0.0);
  vec4 normTexture = texture2D(s_MatTexture, v_texcoord0);
  bool getGLOW = GLOW_PIXEL(glowTexture);
  ...
  bool getMetals = METAL_PIXEL(normTexture);
