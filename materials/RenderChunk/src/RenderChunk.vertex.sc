...
worldPos.y += bI * bX - fogEffect;
 ...
 float cve2 = smoothstep(0.68, 0.05, abs(a_texcoord1.y));
  ...
  vanillaAO = 1.0 - aoFactor * 1.4;
  float fakeAO = clamp(vanillaAO * 0.9, 0.0, 1.0);
  AOm = mix(vec3(1.0,1.0,1.0), vec3(0.3,0.3,0.3), fakeAO);
...
vec3 skyCol = albedo4-0.18;
  ...
  float fogDist = mix(0.0, mix(0.0, 0.7, smtr1), abs(a_texcoord1.y) * AFrain);
  vec4 azifyColor1;
